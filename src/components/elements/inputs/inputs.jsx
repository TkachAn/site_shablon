...
const BaseInput = ({
  ...
  const isValid = required
    ? error === "" && value !== undefined && value !== null && value.toString().trim() !== ""
    : error === "";

  // Register on mount, unregister on unmount
  useEffect(() => {
    if (context && name) {
      context.registerField(name, isValid);
      return () => {
  ...
  useEffect(() => {
    if (name) {
      if (context) {
        context.reportStatus(name, isValid);
      }
      if (onStatusChange) {
        onStatusChange(name, isValid);
      }
    }
  }, [isValid, name, context, onStatusChange]);
...
export const TextAreaInput = forwardRef(
  (
    ...
    const handleBlur = (e) => {
      ...
      if (required) {
        setInternalError(
          cleaned.trim() === "" ? "Поле обязательно для заполнения" : "",
        );
      }
      if (onChange) {
        const event = Object.create(e);
...
export const PriceInput = forwardRef(
  (
    ...
    const currentValue = digits ? String(Number(digits) / 100) : "";
